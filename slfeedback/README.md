...
## Письмо автору вопроса

После сохранения ответа на странице:

```text
/panel/sells/slfeedback/questions/answer/{question_id}/{answer_token}
```

или при обычном сохранении вопроса в backend:

```text
/panel/sells/slfeedback/questions/update/{question_id}
```

сервис сохраняет ответ и отправляет письмо автору вопроса на `question.email`.

Сервис сохранения ответа и запуска уведомления:

```text
services/QuestionAnswerService.php
```

Результат отправки возвращается через enum:

```text
classes/enums/enums/AnswerNotificationStatus.php
```

Сервис отправки письма:

```text
services/QuestionAuthorAnswerMailService.php
```

Шаблон:

```text
views/mail/answer_for_author.htm
```

Mail template:

```text
sells.slfeedback::mail.answer_for_author
```

Чтобы быстрые правки ответа не создавали несколько писем подряд, используется поле:
...
Повторная отправка разрешена только если текущий текст ответа отличается от последней отправленной версии и с последней успешной отправки прошло не меньше 3 минуты. Если специалист сразу исправляет опечатку, ответ сохраняется, но письмо пользователю повторно не отправляется до окончания таймаута. После окончания таймаута повторное сохранение исправленной версии отправит пользователю уже новый текст.
